<div class="tab-alani">
	<ul class="nav nav-pills mb-0" id="pills-tab" role="tablist">
		...
		?>
		<li class="nav-item">
			<a class="nav-link gri alti <?php echo (isset($get["liste"]) AND $get["liste"]==$value["ayar_1"]) ? 'active show':'';?>" href="<?php echo $ayar->getpage('theme').'?include=spacialpackets&liste='.$value["ayar_1"];?>"><i class="<?php echo $value["icon"];?>"></i> <?php echo $value["head"];?></a>
		</li>
		<?php } ?>
	</ul>
</div>
...
<div class="tab-pane fade active show">
	<form class="loftForm" method="POST">
		<input type="hidden" data-loftAction="loftAction" name="olay" value="loftOptions">
		<input type="hidden" name="page" value="theme">
		<input type="hidden" name="loftAction" value="spacialPackageUpdate">
		<input type="hidden" name="spacialPackage" value="<?php echo $value["ayar_1"];?>">
		<div class="row">
			<div class="col-md-8">
				<div class="card">
				...
				<div class="card">
					...
					<div class="card-body">
						<div class="form-row">
							...
							<div class="col-md-12">
								<div class="form-group">
									<label class="font-weight-bold">Paket Başlığı</label>
									<input class="form-control smgin" name="data[item2][head]" placeholder="Başlık Belirle (İndirimde)" value="<?php echo $value["head"];?>" required="">
								</div>
							</div>
							<div class="col-md-12">
								<div class="form-group">
									...
									<div class="input-group mb-2 renkhover">
										<div class="butto mr-1 smgir rengiyazdir" id="spacialColor" style="background: <?php echo $value["color"];?>; width: calc(100% - 10px); text-shadow: none;">
										</div>
										<input type="text" class="form-control renksecici smginx" name="data[item2][color]" onchange="alert();" data-renk="spacialColor" required="" value="<?php echo $value["color"];?>"></div>
										<input type="hidden" data-renkRgb="spacialColor" name="data[item2][colorRgb]" value="<?php echo $value["colorRgb"];?>">
									</div>
								</div>
								<div class="col-md-12">
									...
									<select class="form-control" name="data[item2][fakediscount]">
										<?php for ($i=0; $i < 101; $i++) { ?>
										<option value="<?php echo $i;?>" <?php echo $value["fakediscount"]==$i ? 'selected':'';?>>%<?php echo $i;?> İndirim</option>
										<?php } ?>
									</select>
								</div>
			...
			<div class="col-md-8">
				<div class="tab-pane fade active show">
					<form class="loftForm" method="POST">
						...
						<input type="hidden" name="page" value="theme">
						<input type="hidden" name="loftAction" value="spacialPackageNew">
						<div class="card">
							<div class="card-header">
								<div class="box-title font-weight-bold">Sayfa Bilgileri</div>
							</div>
							<div class="card-body">
								<div class="form-row">
									...
									<div class="col-md-12">
										<div class="form-group">
											<label class="font-weight-bold">Paket Başlığı</label>
											<input class="form-control smgin" name="data[item2][head]" placeholder="Başlık Belirle (İndirimde)" value="" required="">
										</div>
									</div>
									<div class="col-md-12">
										<div class="form-group">
											<label class="font-weight-bold">Paket Rengi</label>
											<div class="input-group mb-2 renkhover">
												<div class="butto mr-1 smgir rengiyazdir" id="spacialColor" style="background: #b32b2b; width: calc(100% - 10px); text-shadow: none;">
												</div>
												<input type="text" class="form-control renksecici smginx" name="data[item2][color]" data-renk="spacialColor" required="" value="#b32b2b"></div>
												<input type="hidden" data-renkRgb="spacialColor" name="data[item2][colorRgb]" value="179, 43, 43">
											</div>
										</div>
										<div class="col-md-12">
											<label class="font-weight-bold">Fake İndirim</label>
											<select class="form-control" name="data[item2][fakediscount]">
												<?php for ($i=0; $i < 101; $i++) { ?>
												<option value="<?php echo $i;?>">%<?php echo $i;?> İndirim</option>
												<?php } ?>
											</select>
										</div>
										<div class="col-md-12">
											<input type="hidden" name="data[item2][package][]" value="0">
											<input type="hidden" name="data[item1]" value="loftSpecialPackage">
