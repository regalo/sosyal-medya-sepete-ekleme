<?php if (isset($post["olay"])) {
	if ($post["olay"]=="intro-butonlar") {
		foreach ($post["howbox"] as $value) {
			$bolumler[$value] = $post["baslik"][$value].'?=?'.$post["icon"][$value];
		}
		$ayar->select('default-slogan');
		$ayar->item5 = $bolumler[1].'=?='.$bolumler[2].'=?='.$bolumler[3];
		$ayar->update();
	} elseif ($post["olay"]=="intro-hizmetler") {
		$ayar->select('default-ustyonlendirme');
		$ayar->item2 = implode(",", $post["item2"]);
		$ayar->update();
	} elseif ($post["olay"]=="paketler-coksatan") {
		$ayar->select('default-coksatan');
		if (is_array($post["item2"])) {
			$ayar->item2 = implode(",", $post["item2"]);
		} else {
			$ayar->item2 = "";
		}		
		$ayar->update();
	}  elseif ($post["olay"]=="default-renk") {
		$ayar->select('default-ozellestirme');
		if (is_array($post["renk"])) {
			$ayar->item2 = implode("-", $post["renk"]);
		} else {
			$ayar->item2 = "";
		}		
		$ayar->update();
	} elseif ($post["olay"]=="default-kutucular") {
		foreach ($post["boxs"] as $value) {
			$ayar->ayar_1 = $value;
			$ayar->id();
	...
	$alert->content = "Değişiklikler Uygulandı";
	$alert->action = "close";
...
<div class="tab-alani">
	<ul class="nav nav-pills mb-0" id="pills-tab" role="tablist">
		<li class="nav-item">
			<a class="nav-link gri active show" id="pills-intro-tab" data-toggle="pill" href="#pills-intro" role="tab" aria-controls="pills-intro" aria-selected="true"><i class="fas fa-bolt"></i> Header</a>
		</li>
		<li class="nav-item">
		...
		<li class="nav-item">
			<a class="nav-link gri" id="pills-contact-tab" data-toggle="pill" href="#pills-contact" role="tab" aria-controls="pills-contact" aria-selected="true"><i class="far fa-id-card"></i> İletişim</a>
		</li>
		<li class="nav-item">
...
<div class="tab-content" id="pills-tabContent">
	<div class="tab-pane fade active show" id="pills-intro" role="tabpanel" aria-labelledby="pills-intro-tab">
											...
											<div class="form-group">
												<div class="input-group">
													<div class="butto butto-light mr-1 smgir">
														<i id="iconView_Nasil<?php echo $sayis;?>" class="<?php echo $nasil[1];?>" aria-hidden="true"></i>
													</div>
													<input class="form-control smginx" id="iconInput_Nasil<?php echo $sayis;?>"  name="icon[<?= $sayis;?>]" value="<?php echo $nasil[1];?>">
													<button type="button"
													class="butto butbor butto-dark icon-modal"
													data-toggle="modal"
													data-target="#iconSec"
													data-icon="<?php echo $nasil[1];?>"
													data-add="Nasil<?php echo $sayis;?>">Icon Seç</button>
												</div>
											</div>
										</div>
										<div class="w-100">
											<div class="form-group">
												<input class="form-control smgin" name="baslik[<?= $sayis;?>]" value="<?php echo $nasil[0];?>" placeholder="Başlık" required=""></div>
											</div>
										</div>
										<?php $sayis++;  } ?>
										<div class="col-md-12">
														...
														foreach (ns_filter('default-tercih-box','list') as $kt) { ?>
															...
															<div class="tab-pane fade active <?php if ($say == 10) echo "show"; ?> w-100" id="kutu<?php echo $say;?>" role="tabpanel" aria-labelledby="pills-kutu<?php echo $say;?>-tab">
																<div class="col-md-12 mb-2">
																	<div class="input-group">
																		<div class="butto butto-light mr-1 smgir">
																			<i id="iconView_Kutu<?php echo $say;?>" class="<?php echo $kt["item4"];?>" aria-hidden="true"></i>
																		</div>
																		<input class="form-control smginx" id="iconInput_Kutu<?php echo $say;?>"  name="item4[<?=$kt["ayar_1"];?>]" value="<?php echo $kt["item4"];?>">
																		<button type="button"
																		class="butto butbor butto-dark icon-modal"
																		data-toggle="modal"
																		data-target="#iconSec"
																		data-icon="<?php echo $kt["item4"];?>"
																		data-add="Kutu<?php echo $say;?>">Icon Seç</button>
																	</div>
																</div>
																<div class="col-md-12">
									...
									<div class="row">
										<div class="col-md-8">
											<div class="card">
												...
												<div class="card-body">
													<form id="default-c_detail" method="POST" action="" onsubmit="fastpost('default-c_detail','ajaxout'); return false;">
														<?php $ayar->select("default-c_detail","create");?>
														<input type="hidden" name="page" value="theme">
														<input type="hidden" name="olay" value="default-c_detail">
														<div class="row">
															<div class="form-group col-md-12">
																<label><b>Durum</b></label>
																<select class="form-control" name="statu">
																	<option value="0" <?php echo ns_filter('default-c_detail','statu')=="0" ? 'selected':'';?>>Pasif</option>
																	<option value="1" <?php echo ns_filter('default-c_detail','statu')=="1" ? 'selected':'';?>>Aktif</option>
																</select>
															</div>
															<div class="form-group col-md-4">
																<label><b>Firma Adı</b></label>
																<input type="text" class="form-control" name="item2" placeholder="Firma Adı Giriniz" value="<?php echo ns_filter('default-c_detail','item2');?>">
															</div>
															<div class="form-group col-md-4">
																<label><b>Telefon Numarası</b></label>
																<input type="text" class="form-control" name="item3" placeholder="Telefon Numarası Giriniz" value="<?php echo ns_filter('default-c_detail','item3');?>">
															</div>
															<div class="form-group col-md-4">
																<label><b>E-Posta Adresi</b></label>
																<input type="text" class="form-control" name="item4" placeholder="E-Posta Giriniz" value="<?php echo ns_filter('default-c_detail','item4');?>">
															</div>
															<div class="form-group col-md-12">
																<label><b>Adres</b></label>
																<textarea type="text" class="form-control" name="item5" placeholder="Adres Giriniz"><?php echo ns_filter('default-c_detail','item5');?></textarea>
															</div>
															<div class="col-md-12">
										...
										<div class="col-md-4">
											<div class="alert alert-danger">
												...
												<p class="mb-0">İletişim sayfasına iletişim bilgilerinizi(Firma adı, telefon, mail, adres) eklemek için iletişim detaylarını aktif hale getirip bilgilerinizi girin.</p>
												<br>
												<p class="mb-0">Eklemedik istemediğiniz bilgiler için alanı boş bırakabilirsiniz.</p>
											</div>
										</div>
									</div>
								</div>
