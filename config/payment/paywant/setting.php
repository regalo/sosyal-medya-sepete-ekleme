<div class="tab-pane fade <?php if(ns_filter($method) == "paywant") { echo "active show";}?>" id="pills-<?php echo $value["folder"].'-'.$method;?>" role="tabpanel" aria-labelledby="pills-<?php echo $value["folder"].'-'.$method;?>-tab">
    <form id="<?php echo $value["folder"].'-'.$method;?>" method="POST" action="" onsubmit="fastpost('<?php echo $value["folder"].'-'.$method;?>','ajaxout'); return false;">
        ...
        <div class="row">
            <div class="form-group col-md-6">
                ...
                <input class="form-control" name="item3" required="" value="<?php if(ns_filter($method,"item2")=="paywant") { echo ns_filter($method,"item3");}?>">
            </div>
            <div class="form-group col-md-6">
                <label class="form-control-label font-weight-bold">Api Secret</label>
                <input class="form-control" name="item4" required="" value="<?php if(ns_filter($method,"item2")=="paywant") { echo ns_filter($method,"item4");}?>">
            </div>
            <div class="form-group col-md-12">
                ...
                <input class="form-control" readonly="" value="<?php echo ns_filter('siteurl').'payment/paywant/';?>">
            </div>
            <div class="form-group col-md-12">
            ...
            <div class="col-md-12 text-right mt-3">
                <input type="hidden" name="item2" value="paywant">
                <?php
                if($method=="onlinepay")
                    $odeme_methodu = 2;
                else if($method=="mobilepay")
                    $odeme_methodu = 1;
                else if($method=="havalepay")
                    $odeme_methodu = 3;
                ?>
                <input type="hidden" name="item5" value="<?php echo $odeme_methodu;?>">
